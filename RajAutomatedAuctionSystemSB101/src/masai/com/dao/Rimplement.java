...
public class Rimplement implements Roperation {
	...
	public List<Seller> GetAllData() throws SellException {
		...
		try (Connection c=d.db()){
			
			PreparedStatement p=c.prepareStatement("select * from  seller");
			ResultSet rs=p.executeQuery();
			while(rs.next()) {
				 int id=rs.getInt("sellid");
				 String name=rs.getString("productname");
				 String pr=rs.getString("productcat");
				 int qn=rs.getInt("qn");
				 int price=rs.getInt("price");
				 int rid=rs.getInt("rid");
				 Seller s=new Seller(id, name, pr, qn, price, rid) ;
				 sl.add(s);
